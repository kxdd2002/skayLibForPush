...
class PacketWriter {
    ...
    */ 
    ...
    void startKeepAliveProcess() {
        ...
        AliveWakeLock awl = SmackConfiguration.getAliveWakeLock();
        if (keepAliveInterval > 0) {
            KeepAliveTask task;
            if(awl!=null){
            	startKeepAliveTask(keepAliveInterval,awl);
//            	task = new KeepAliveTask(keepAliveInterval,awl);
            }else{
            	task = new KeepAliveTask(keepAliveInterval);
    ...
    public void shutdown() {
        ...
        AliveWakeLock awl = SmackConfiguration.getAliveWakeLock();
        if(awl!=null){
        	unRegisterKeepAliveReceiver(awl.getContext());
        }
    }
    ...
    private void startKeepAliveTask(long delay,AliveWakeLock awl){
    	registerKeepAliveReceiver(awl.getContext());
    	startKeepAliveAlarm(awl.getContext(),delay);
    }
	 ...
	 * 
	 */
	private void startKeepAliveAlarm(Context service,long delay){
		AlarmManager alarmManager = (AlarmManager) service.getSystemService(Context.ALARM_SERVICE);
		Intent intent = new Intent("alarm.fanxer.xmpp.keepAlive");
		// 创建封装BroadcastReceiver的pendingIntent对象
		PendingIntent pendingIntent = PendingIntent.getBroadcast(service, 0,
				intent, 0);
		// 开始定时器，每delay secs执行一次
		alarmManager.setInexactRepeating(AlarmManager.RTC_WAKEUP, 15 * 1000 , delay,
				pendingIntent);
	}
    ...
    private void registerKeepAliveReceiver(Context service){
		IntentFilter filter = new IntentFilter("alarm.fanxer.xmpp.keepAlive");
		service.registerReceiver(mReceiver, filter);
		addReg();
	}
    
    private void unRegisterKeepAliveReceiver(Context service){
    	removeReg();
    	if(getRegCount()<=0){
    		try {
	 ...
	 * 检测并发送心跳包
	 */
	private void checkHeartMessage(){
		Thread t = new Thread(new Runnable(){
			...
			public void run() {
				...
				int keepAliveInterval = SmackConfiguration.getKeepAliveInterval();
		        AliveWakeLock awl = SmackConfiguration.getAliveWakeLock();
				sendKeepAlivePacket(keepAliveInterval,awl);
			}});
		t.setName("Smack Keep Alive");
		t.start();
	}
	
	public void sendKeepAlivePacket(long delay,AliveWakeLock awl){
    	synchronized (writer) {
            // Send heartbeat if no packet has been sent to the server for a given time
            if (System.currentTimeMillis() - lastActive >= delay) {
                try {
                	if(awl!=null){
               		 	awl.acquireCpuWakeLock();
               	 	}
                    writer.write(" ");
                    writer.flush();
                }
            	...
            	if(awl!=null){
           		 	awl.onError("delay time:"+(System.currentTimeMillis() - lastActive));
           	 	}
            }
        }
    }
    ...
    private class KeepAliveTask implements Runnable {
        ...
        public KeepAliveTask(int delay,AliveWakeLock awl) {
            this.delay = delay;
            this.awl = awl;
        }
...
//                // Do nothing
//            	if(awl!=null){
//            		awl.onError("InterruptedException0:"+ie.toString());
//            	}
//            }
...
//                    Thread.sleep(delay);
//            		long delayTime = (System.currentTimeMillis() - lastActive)>0?delay-(System.currentTimeMillis() - lastActive):delay;
                	SystemClock.sleep(delay);
//                }
//                catch (InterruptedException ie) {
//                    // Do nothing
//                	if(awl!=null){
//                		awl.onError("InterruptedException1:"+ie.toString());
//                	}
//                }
            }
            if(awl!=null){
       		 	awl.onError("thread stop becauce done?:"+done+";thread:"+(keepAliveThread == thread));
       	 	}
        }
    }
}