...
import org.jivesoftware.smack.packet.Message.Delay;
import org.jivesoftware.smack.packet.Message.Template;
...
public class PacketParserUtils {
		...
		while (!done) {
			...
			if (eventType == XmlPullParser.START_TAG) {
				...
				if (elementName.equals("subject")) {
					...
					message.setDelay(parseDelay(parser));
				} else if (elementName.equals("template")) {
					Template t = parseTemplate(parser);
					t.setSpace(namespace);
					message.setTemplate(t);
			...
			if (eventType == XmlPullParser.START_TAG) {
				...
				// Otherwise, it must be a packet extension.
					...
					registration.addExtension(PacketParserUtils
							.parsePacketExtension(parser.getName(), parser
									.getNamespace(), parser));
				}
			} else if (eventType == XmlPullParser.END_TAG) {
				if (parser.getName().equals("query")) {
	...
	private static Delay parseDelay(XmlPullParser parser) throws IOException,
			XmlPullParserException {
		Delay bind = new Delay();
		for (int i = 0; i < parser.getAttributeCount(); i++) {
			String attributeName = parser.getAttributeName(i);
			if ("stamp".equals(attributeName)) {
				bind.setStamp(parser.getAttributeValue(i));
			} else if ("from".equals(attributeName)) {
				bind.setFrom(parser.getAttributeValue(i));
			}
		}
		return bind;
	}

	private static Template parseTemplate(XmlPullParser parser) throws IOException,
			XmlPullParserException {
		Template template = new Template();
		for (int i = 0; i < parser.getAttributeCount(); i++) {
			String attributeName = parser.getAttributeName(i);
			if ("name".equals(attributeName)) {
				template.setName(parser.getAttributeValue(i));
			} 
		}
		return template;
	}
			...
			if (eventType == XmlPullParser.START_TAG) {
				...
				object.getClass().getMethod(
						"set" + Character.toUpperCase(name.charAt(0))
								+ name.substring(1), propertyType).invoke(
						object, value);
			} else if (eventType == XmlPullParser.END_TAG) {
				if (parser.getName().equals(elementName)) {
					done = true;
